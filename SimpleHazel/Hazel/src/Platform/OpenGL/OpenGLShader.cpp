...
namespace Hazel {
	...
	void OpenGLShader::CompileOrGetVulkanBinaries(const std::unordered_map<GLenum, std::string>& shaderSources)
	{
		...
		for (auto&& [stage, source] : shaderSources)
		{
			...
			{
				/*shaderc::SpvCompilationResult shader_module = compiler.CompileGlslToSpv(source, Utils::GLShaderStageToShaderC(stage), m_FilePath.c_str(), options);
				...
				shaderData[stage] = std::vector<uint32_t>(shader_module.cbegin(), shader_module.cend());*/
	...
	static bool report_shader_info_log(std::string const& filename, GLuint shaderObject)
	{
		GLint infoLen = 0;
		...
		if (infoLen > 1)
		{
			...
			glGetShaderInfoLog(shaderObject, infoLen, NULL, infoLog.data());
			HZ_CORE_ERROR("Error compiling shader:({0})\n{1}", filename,infoLog.c_str());
			return true;
		}
		return false;
	}
	...
	void OpenGLShader::CreateProgram()
	{
		...
		for (auto&& [stage, source] : m_OpenGLSourceCode)
		{
			...
			std::vector<const char*> sources{ source.data() };
			glShaderSource(shaderID, 1, sources.data(), nullptr);
			glCompileShader(shaderID);

			GLint compiled;
			glGetShaderiv(shaderID, GL_COMPILE_STATUS, &compiled);
			if (!compiled)
				report_shader_info_log(m_FilePath, shaderID);
			else
				glAttachShader(program, shaderID);
		}
		...
		if (isLinked == GL_FALSE)
		{
			...
			HZ_CORE_ERROR("Shader linking failed ({0}):\n{1}", m_FilePath, maxLength?infoLog.data():"unknown error");
