...
ImGuiMultiSelectData *ImGui::BeginMultiSelect(ImGuiMultiSelectFlags flags, void *range_ref, bool range_ref_is_selected)
{
	...
	if (state->In.RequestSelectAll)
		printf("[%05d] BeginMultiSelect: RequestSelectAll\n", g.FrameCount);
#endif
...
#ifdef IMGUI_DEBUG_MULTISELECT
	if (state->Out.RequestClear)
		printf("[%05d] EndMultiSelect: RequestClear\n", g.FrameCount);
	if (state->Out.RequestSelectAll)
		printf("[%05d] EndMultiSelect: RequestSelectAll\n", g.FrameCount);
	if (state->Out.RequestSetRange)
		printf("[%05d] EndMultiSelect: RequestSetRange %p..%p = %d\n", g.FrameCount, state->Out.RangeSrc, state->Out.RangeDst, state->Out.RangeValue);
#endif
